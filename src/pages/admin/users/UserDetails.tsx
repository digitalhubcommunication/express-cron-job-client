...
import { useGetSingleUserQuery } from "@/redux/features/adminActions/adminActions";
...
//   handlers
const handleStatusChange = (id:string, type:"default"|"manual", status:'enabled'| 'disabled')=>{

}

const updateUserInfo = (data:any)=>{
        console.log(data,' query to perform');
}



// callback functions
const status_CB = ()=>updateUserInfo({status: data?.user?.status ==="enabled"?"disabled":"enabled"});
const ATAMD_CB = ()=>updateUserInfo({allowedToAddManualDomains: !data?.user?.subscription?.allowedToAddManualDomains})

// conditional return
  if(isFetching) return <PageLoading />

  if(!data?.user) return <ErrorMessage key="NO_USER_FOUND" msg="No user found" />

  const firstIndexes = data?.user?.defaultDomains?.length || 0;

  return <DashboardContainer className="py-10">
         <h6 className="font-semibold ecj_fs-xl mb-2">{data?.user?.name || ''}</h6>
         <p className="mt-1"><span className="font-semibold">Username :</span> {data?.user?.username || ""}</p>
         <p className="mt-1"><span className="font-semibold">Role :</span> {data?.user?.role || ""}</p>
         <p className="mt-1"><span className="font-semibold">Mobile :</span> {data?.user?.mobile || ""}</p>
         ...
         <p className="mt-1"><span className="font-semibold">Total Manual Cron : </span> {data?.user?.manualDomains?.length || 0}</p>
         <p className="mt-1"><span className="font-semibold">Subscription :</span> {data?.user?.subscription?.name || ""}</p>
        <p className="mt-1"><span className="font-semibold">Expiry Date :</span> {data?.user?.packageExpiresAt ? getExpiryText(data?.user?.packageExpiresAt).date:  'Not found'}</p>



    {/* ==== */}
    <p className="mt-1"><span className="font-semibold">Account created on :</span> {data?.user?.createdAt ? getExpiryText(data?.user?.createdAt).date:  'Not found'}</p>
    <br />
  
   
    <h6 className="mb-3 font-semibold">Domains</h6>
            <div className="overflow-x-auto border-t border-b max-h-[500px] lg:border border-slate-300 rounded-md">
              ...
              <table className="w-full min-w-[750px] lg:min-w-[800px] text-left border-collapse hidden sm:table">
                ...
                <tbody>
                  {data?.user?.defaultDomains?.length && (data?.user?.defaultDomains as TDomain[])?.map((domain, i) => (
                    <tr
                      key={domain._id}
                    ...
                    >
                      ...
                      <td className="max-w-[200px] px-3 py-2">{data?.user?.subscription?.intervalInMs || ""}</td>
                      <td className="max-w-[200px] px-3 py-2">
                        <button  onClick={()=>handleStatusChange(domain._id, "default", domain.status === "enabled" ? "disabled" : "enabled")} className=" underline text-blue-600">
                ...
                {data?.user?.defaultDomains?.length && (data?.user?.defaultDomains as TDomain[])?.map((domain, i) => (
                  ...
                  >
                    ...
                    <div className="flex mt-1">
                      ...
                      <button
                       onClick={()=>handleStatusChange(domain._id, "default", domain.status === "enabled" ? "disabled" : "enabled")}
